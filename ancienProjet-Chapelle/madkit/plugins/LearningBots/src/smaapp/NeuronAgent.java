...
public void initialisation(String g,String n, NeuronScheduler nsch) 
	{	
		...
		neuronName=n;
		this.nsch=nsch;
		System.out.println("Agent neurone initialisé ... ("+simulationGroup+" "+neuronName+" "+this.nsch+")");
	}
...
public void setActors(Vector<NeuronAgent> MotorGroup)
{
	...
	{
		FuzzyFilter ff = new FuzzyFilter(FuzzyFunc.LINE,FuzzyFilter.nb_intervales_def);
		ActorFilters.add(ff);
		System.out.println(ff.toString());
		/*for (int j=0; j<=5 ; j++)
		{	float value=((float)j)/5;
			int interv=ff.appartenance(value);
			float ap=ff.appartenance(value,interv);
			System.out.println(value+ " appartient à l'intervale "+interv+" µ(value)="+ap);
		}*/
	}
	// Puis créer matrice apprentissage
	Brain = new LearnModule (1,(int)Math.pow(FuzzyFilter.nb_intervales_def,Actors.size()),/*1,*/2);;
	Brain.InitMAct();
	// save / load brain
	if (!RmcLauncherApp.is_applet)
	{	String rsrc=(nsch.simgroup+"."+nsch.neurongroup+"."+this.getLabel()+".mact.txt");
		if (1==1)
		{	
...
public void delibere()
{
	if (persist<=0)
	{
		...
		if (last_action!=-1)
		{
			...
			// Calcul de la récompense qui tient compte du delta et de la valeur initiale de sat
			...
			// Calcul classique de la nouvelle valeur en fnct de l'ancienne et de reward:
			//float oldvalue = Brain.GetValue(0,last_action,0);
			//float newv = (beta * oldvalue)+(1-beta)*reward;
			// La même chose en une seule ligne:
			float newv = (beta * Brain.GetValue(0,last_action,0))+(1-beta)*reward;
			...
			// La même chose en une seule ligne:
			Brain.SetValue(0,last_action,1,Brain.GetValue(0,last_action,1)+1);
			this.saveBrain("s"+nsch.simgroup+"."+nsch.neurongroup+"."+this.getLabel()+".mact.txt");
		}
		...
		{
			poids=(Brain.GetValue(0,i,0)+1)/(Brain.GetValue(0,i,1)+1);
			if (poids>poids_max) {poids_max=poids; action_max=i;}
		}
	...
	{
		persist--;
		sat_moy=(sat_moy*((float)sat_moy_nb) + ((float)nsch.getDriveByName("SatP").getLastValue()))/(sat_moy_nb+1);
		sat_moy_nb++;
	}
	...
	sendMessage(Actors.elementAt(0).getAddress(),new NeuralMessage( (double)(fq.b.noyau+fq.b_pre_noyau())/2 ));
	fq = ActorFilters.elementAt(1).intervale(MotRAct+1);
	sendMessage(Actors.elementAt(1).getAddress(),new NeuralMessage( (double)(fq.b.noyau+fq.b_pre_noyau())/2 ));
...
public NeuronAgent getActor(int i)
{
	...
	{
		System.out.println("NeuronAgent.getActors(i): indice i hors limites");
		return null;
	}
...
/*
 ...
 /** Asks the agent to present itself through the debug window (System.out.println)*/
	...
	{	if (m.getClass()==NeuralMessage.class)
		{
			...
			{	
				...
				if (first_value==-1) {first_value=last_value;}
			}
			/*
		...
		else
		{
			...
			else // if (m.getClass()==AckCreatedMessage.class) 
			{
				...
				{
					System.out.println("NeuronAgent ("+this.label+") a reçu un string message");
				}
				else // if (m.getClass()==StringMessage.class)
				{
					System.out.println("NeuronAgent ("+this.label+") a reçu un message de type inconnu ("+m.getClass()+")");
				} //else // if (m.getClass()==StringMessage.class)
			} //else // if (m.getClass()==AckCreatedMessage.class)
		}
...
{
	if (Brain==null)
	{	// 5 => Pas, Peu, Moyen, Beaucoup, Tout
		Brain=new LearnModule(nsch.getSensorSize()*5,nsch.getActorSize()*5,/*1,*/2);
		Brain.InitMAct();
		Brain.setLabel(this.getLabel());
		//Brain.AfficheMAct();
	}
	if (this.getLabel()=="Faim")
	{ // REVIENDRE
...
/** Function called by the RunMe() if the neuron it applied to is a DriveManager */
 public String runMeDriveMan()
 {
		if (drivenumber==0) {drivenumber=nsch.getDriveSize();}
		if (Brain==null)
		{
		...
		if (Phase==PHASE_BABBLING)
		{
			...
			else // if (Babble==null)
			{
				...
				if(Babble.Babbler_State==Babble.BABBLER_WORKING)
                {
    				...
    				sendMessage(atemp.getAddress(),new NeuralMessage(vtemp));
	                    ...
	                    if (natmp==null)
	                    	{
			                    ...
			                    for(int i = 0; i < Babble.MotorGroup.size(); i++)
			                        s += (((NeuronAgent)Babble.MotorGroup.get(i)).getLabel());
			                    sendMessage(nsch.getAddress(),new CreateAgentMessage(nsch.neurongroup,s,Babble.MotorGroup));
 ...
 public void load(BufferedReader dip)
 {
 	try
 	  {	String s=null;
 		...
 		s=dip.readLine();System.out.println(dbug+s);	simulationGroup=s;
 		s=dip.readLine();System.out.println(dbug+s);	neuronName=s;
 		s=dip.readLine();System.out.println(dbug+s); last_value=Float.valueOf(s).floatValue();
 		s=dip.readLine();System.out.println(dbug+s); if (s.compareTo("null")==0) {last_ds=null;} else {last_ds=s;}
 		s=dip.readLine();System.out.println(dbug+s); if (s.compareTo("null")==0) {label=null;} else {label=s;}
 		s=dip.readLine();System.out.println(dbug+s); type=(Integer.valueOf(s)).intValue();
 		s=dip.readLine();System.out.println(dbug+s); inhibit=(Integer.valueOf(s)).intValue();
 ...
 public void save (FileWriter dip)
 {
 	...
 	dip.write(dtos(last_value));dip.write("\n");
 	if (last_ds==null) {last_ds=new String("null");}
 	dip.write(last_ds);dip.write("\n");
 	if (label==null) {label=new String("null");}
 	dip.write(label);dip.write("\n");
 	//dip.write(nsch);dip.write("\n");
 	dip.write(itos(type));dip.write("\n");
 	dip.write(itos(inhibit));dip.write("\n");
 ...
 public void save(FileWriter dip)
 {
 	...
 	dip.write(simulationGroup);dip.write("\n");
 	dip.write(neuronName);dip.write("\n");
 	dip.write((new Double(last_value)).toString());dip.write("\n");
 	if (last_ds==null) {last_ds=new String("null");}
 	dip.write(last_ds);dip.write("\n");
 	if (label==null) {label=new String("null");}
 	dip.write(label);dip.write("\n");
 	...
 	dip.write((new Integer(inhibit)).toString());dip.write("\n");
 	//dip.write(inhibit);dip.write("\n");
 	Brain.save(dip);
 	}
       catch (Exception e)
