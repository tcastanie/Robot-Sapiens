...
public class NeuronScheduler extends Scheduler
{
 ...
 /** Crée un nouveau NeuronScheduler dans la simulation simgroup
 pour le groupe de neurones neurongroup **/
 public NeuronScheduler(String simgroup,String neurongroup) //,RobotAppEnv w)
	{
	//world=w;
 ...
 {	System.out.println("Activation neuroscheduler dans groupe "+simgroup+" & "+neurongroup);
	neurlive = new TurboMethodActivator("runMe",neurongroup,"neuron");
	addActivator(neurlive);
	
	sensorObserver = new TurboMethodActivator("observe",neurongroup,"sensor observer");
	...
	requestRole(simgroup, "scheduler",null);
 ...
 public void live() //throws ThreadDeath
  {
	...
	{	
		...
		while (m!=null)
		{
			System.out.println("Nsched:J'ai reçu un message de type: "+m.getClass().toString());
			if (m.getClass()==CreateAgentMessage.class)
			{
				System.out.println("C'était bien une demande de création ("+m+")");
                ...
                System.out.println("Nsch:neurone "+mc.getlabel()+" activé");
			...
			else
			{
				...
				if (!messageread)
				{
					System.out.println("NSched:received:"+lastmessage.getString());
				}
		...
		{	// On ne passera ici qu'au début de la simulation, avant que l'on
			// ai pressé la première fois sur play/pause ... Cette pause permet
			// d'éviter que le simulateur utilise 100% du CPU à boucler dans le
  ...
  /** Renvoie le nombre de Sensor enregistrés
  (identique à getSensorLength()) */
  ...
  {
      System.out.println((new StringBuilder()).append("(NSched):").append(neuronagent).append(" registered as Neuron").toString());
  ...
  /** Renvoie le nombre d' Actor enregistrés
  (identique à getActorLength()) */
  ...
  /** Enregistre l'agent na comme étant un DriveManager (gestionnaire d'émotions) */
  ...
  {
  	return ((NeuronAgent)(getSensorActor(DriveManagers,i,TYPE_DRIVE+TYPE_DRIVE_MANAGER)));
  }
  
  /** Récupère le i ieme Agent de type TYPE dans le vecteur v
  (fonction utilisée par getSensor(...) getActor(...)
  getDrive(...) getDriveManager(...) */
  private NeuronAgent getSensorActor(Vector v,int i,int TYPE)
  {
  	if (i>=v.size())
  	{	String dbmsg="";
  		if (TYPE==TYPE_SENSOR) {dbmsg="Sensor";}
  		if (TYPE==TYPE_ACTOR) {dbmsg="Actor";}
  		if (TYPE==TYPE_DRIVE) {dbmsg="Drive";}
  		if (TYPE==TYPE_DRIVE_MANAGER) {dbmsg="DriveManager";}
  		if (TYPE==(TYPE_DRIVE+TYPE_DRIVE_MANAGER)) {dbmsg="Drive+DriveManager";}
  		System.out.println("Indice hors limites (NSched): get"+dbmsg+"("+i+") -(max="+(v.size()-1)+")");
  		return null;
  	}
  ...
  {	int vtemp;
  	...
  	{	/*stemp = ((NeuronAgent)(v.elementAt(i))).getLabel();
  		vtemp=(stemp.compareTo (name));
  		*/
  		vtemp=((((NeuronAgent)(v.elementAt(i))).getLabel()).compareTo (name));
  		
  		if (vtemp==0)
  		{	//System.out.println("Neurone: (i="+i+")(name="+stemp+") (compareTo="+vtemp+")(name="+name+")");
  			n=i;
  			break;
  		}
  	}
  	...
  	if (n>=0)
  	{	//System.out.println("Agent Existant (NSched): nom ("+name+") non trouvé (n="+n+")");
  		return ((NeuronAgent)(getSensorActor(v,n,TYPE)));
  	}
  	/*else
  	{*/
  		System.out.println("Agent Inexistant (NSched): nom ("+name+") non trouvé (n="+n+")");
  		return null;
  	//}
  }
  	...
  	if (v!=null)
  	{
	  	if (v.size()>0)
	  	{
		  	...
		  	{	na=((NeuronAgent)(v.elementAt(i)));
		  		System.out.println("Killing "+na.getLabel()+"("+na.neuronName+")");
		  		na.end();
		  		killAgent(na);
		  	} // for (int i=0;i<v.size();i++)
		} // if (v.size()>0)
...
{ return ((new Float(n)).toString()); }
  ...
  {	String dbug=new String("NSch.SaveBrain:");
  	...
  	try {dip.write(itos(nbsave)+"\n");}
  	catch (Exception e) {System.err.println(dbug+""+e.getMessage());}
  	
  	for (int i=0;i<Neurons.size();i++)
  	{	na=((NeuronAgent)(Neurons.elementAt(i)));
  		System.out.println(dbug+"Saving "+na.getLabel()+"("+na.neuronName+")");
  		na.save(dip);
  	}
