...
public class PropertyPane extends JPanel implements ActionListener{
	...
	public boolean getSkinUse(){return propertyView.getSkinUse();}
	public void setSkinUse(boolean b){
		propertyView.setSkinUse(b);
	}

	public String getSkinTheme(){return propertyView.getSkinTheme();}
	public void setSkinTheme(String b){
		propertyView.setSkinTheme(b);
	}
...
class PropertyEdit extends JPanel implements ActionListener, ChangeListener{
	...
	JCheckBox useWallpaper, useSkin;
	JTextField wallpaperPath;
	JTextField themePath;
	JComboBox wallpaperPos, themePos;
	JButton browse, selectColor, browseTheme;
	JPanel pColor;
	...
	public PropertyEdit(PropertyPane propertyPane){
		...
		String[] strs = {PropertyDesktop.CENTER, PropertyDesktop.STRETCH};
		...
		pane = new JPanel(new BorderLayout());
		pane2 = new JPanel();
		pane2.add(new JLabel("Background Color :"));
		pColor = new JPanel();
		...
		if (propertyPane.getSkinUse())
			useSkin = new JCheckBox("Use skin", true);
		else
			useSkin = new JCheckBox("Use skin", false);
		useSkin.addChangeListener(this);
		pane2.add(useSkin);
		String[] themes = this.loadSkinThemes();
		themePos = new JComboBox(themes);
		themePos.setSelectedItem(propertyPane.getSkinTheme());
		themePos.addActionListener(this);
		pane2.add(themePos);
//		themePath = new JTextField(20);
////		if (propertyPane.getWallpaperPath() != null)
////			wallpaperPath.setText(propertyPane.getWallpaperPath());
//		themePath.setEditable(false);
//		pane2.add(themePath);
//		browseTheme = new JButton("Browse");
//		browseTheme.addActionListener(this);
//		pane2.add(browseTheme);
		pane.add(pane2, BorderLayout.CENTER);
		add(pane);
	...
	public void actionPerformed(ActionEvent e){
		...
		if (o == selectColor){
			...
			Color color = JColorChooser.showDialog(this, "Please choose the background color", pColor.getBackground());
			if (color != null)
		...
		if (o == themePos){
			propertyPane.setSkinTheme(themePos.getSelectedItem().toString());
		}
	}
	public void stateChanged(ChangeEvent e){
		...
		if (o == useWallpaper){
			...
			propertyPane.setSkinUse(useSkin.isSelected()); 
		}
	}
	
	String[] loadSkinThemes(){
		File skinDir = new File(Madkit.libDirectory+File.separatorChar+"skins");
		String[] res = skinDir.list(
				new FilenameFilter(){
					String extens = "zip";
...
class PreviewScreen extends JPanel{
	...
	public void paint(Graphics g){
		...
		if (wallpaperPath != null && useWallpaper){
			int X, Y, W, H;
			if (wallpaperPos.equalsIgnoreCase(PropertyDesktop.CENTER)){
				int i, j;
