...
//		map.put("autoRemove", new Boolean(true));
	...
	public void activate(){
		createGroup(false,"public","system",null,null);
...
//System.out.println("\t\t"+o);
	...
	protected void handleMessage(KernelMessage message){
		...
		if (message.getOperation() == Kernel.DUMP_COMMUNITIES){
			...
			if (hasGUI()){
				AgentEventAddTo event = new AgentEventAddTo(AgentEvent.ADD_TO, new Date().getTime(),  agr.getAgent());
				event.setColor((Color)this.map.get("colorIn"));
				HashMap map = new HashMap();
			...
			AGRTrio agr = (AGRTrio) message.getArgument();
    		if (hasGUI()){
				AgentEventRemoveFrom event = new AgentEventRemoveFrom(AgentEvent.REMOVE_FROM, new Date().getTime(),  agr.getAgent());
				event.setColor((Color)this.map.get("colorOut"));
				HashMap map = new HashMap();
				map.put("Community", agr.getCommunity());
				map.put("Group", agr.getGroup());
				map.put("Role", agr.getRole());
...
class GraphicGroupObserverGUI extends JPanel implements ActionListener{
	...
	public HashMap getMap(){ return agent.getMap(); }
	public void addExistingAgent(AgentAddress agent, HashMap map){ graphic.addExistingAgent(agent, map); }
	public void addMemberRole(AgentEvent event, HashMap map){ graphic.addMemberRole(event, map); }
	public void removeFrom(AgentEvent event, HashMap map){ graphic.removeFrom(event, map); }
	public void removeAllFrom(AgentEvent event, HashMap map){ graphic.removeAllFrom(event, map); }
	public void killAgent(AgentEvent event){ graphic.killAgent(event); }
	public void addMessage(AgentEvent event){ graphic.addMessage(event); }
	public void addBroadcastMessage(AgentEvent event){ graphic.addBroadcastMessage(event); }
}
...
class GraphicGroupObserverGUIPane extends JPanel implements MouseMotionListener, MouseListener{
	...
	protected void addAgentFirst(AgentSpy agentSpy){
	...
	protected void addAgentLast(AgentSpy agentSpy){
	...
	protected void removeAgent(AgentSpy agentSpy){
	...
	protected void removeEvent(AgentEvent event){
	...
	public int getPos(AgentAddress ad){
	    ...
	    while (it.hasNext()){
	    	AgentSpy agentSpy = (AgentSpy)it.next();
	    	if (agentSpy.getAgentAddress() == ad) return i;
	    	i++;
	    }
	    return -1;
	}
	public void removeKilledAgent(){
		Iterator it = agents.iterator();
	    while(it.hasNext()){
	    	AgentSpy agentSpy = (AgentSpy)it.next();
	...
	public void setCurrentEvent(AgentEvent event){ currentEvent = event; repaint(); }
	public AgentEvent getCurrentEvent(){ return currentEvent;}
	public LinkedList getListAgent(){ return agents; }
	public LinkedList getListEvent(){ return events; }
	public int getAgentWidth(){ return agentW; }
	public int getHeaderHeight(){ return agentH; }
	public int getStaticStep(){ return staticStep; }
	public int getMargeX(){ return margeX; }
	public int getMargeY(){ return margeY; }
}
...
class GraphicHeader extends JPanel{
	...
	public void paint(Graphics g){
	    ...
	    while (it.hasNext()){
			...
			float drawPosY = Y;
			String str = agentSpy.getAgentAddress().getName();
			if (str.length()>0){
			...
			if (rect != null) g2D.setClip((int)rect.getX(), (int)rect.getY(), (int)rect.getWidth(), (int)rect.getHeight());
...
class GraphicLeft extends JPanel{
	...
	public Dimension getPreferredSize(){ return new Dimension(20,50); }
}
...
class GraphicDown extends JPanel{
	...
	public Dimension getPreferredSize(){ return new Dimension(50,30); }
}
...
class AgentSpy{
	...
	public AgentAddress getAgentAddress(){ return (this.agentAddress); }
	public void setName(AgentAddress agentAddress){ this.agentAddress = agentAddress; }
	...
	public AgentEvent getComeEvent(){ return (this.comeEvent); }
	...
	public int getNbMap(){ return list.size(); }
	public AgentEvent getQuitEvent(){ return (this.quitEvent); }
	public void setQuitEvent(AgentEvent quitEvent){
		this.quitEvent = quitEvent;
		OPoint p = (OPoint)(beginEndEvent.getLast());
		p.o2 = quitEvent;
	}
	public void setKilled(boolean bool){ killed = bool; }
	...
	public boolean SpiedAt(int step){
	    ...
	    while (it.hasNext()){
	    	...
	    	if (p.o2 == null){
	    		...
	    		if (step >= parent.getListEvent().indexOf(p.o1) && step <= parent.getListEvent().indexOf(p.o2)) return true;
	    	}
	    }
...
abstract class AgentEvent{
	...
	public AgentAddress getSource(){ return source; }
	public AgentAddress getDest(){ return dest; }
	public void setColor(Color color){ this.color = color; }
...
class AgentEventSimple extends AgentEvent{
	...
	public boolean contain(GraphicGroupObserverGUIPane parent, int step, int X, int Y){ return false; }
...
class AgentEventRemoveFrom extends AgentEvent{
	public AgentEventRemoveFrom(int event, long date, AgentAddress source){
		init(event, date, source, null, null, null, null);
		color = Color.orange;
	}
	public String toString(){
		return "Agent " + source.toString() + " quit a group or a role";
	}
	public boolean contain(GraphicGroupObserverGUIPane parent, int step, int X, int Y){
   		int x = (int)((parent.getPos(this.getSource()) + 0.5) * parent.getAgentWidth()) - 10;
   		int y = parent.getMargeY() + step*parent.getStaticStep() - 2;
