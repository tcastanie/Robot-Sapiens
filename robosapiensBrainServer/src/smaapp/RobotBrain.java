...
public class RobotBrain extends RobotAppPhy {
	...
	public void ChoixActionNonAltruisme() {
		...
		case 0: { // calcul dir alea
			...
			if (Math.random() > 0.8) {
				...
				// double alp=5*Math.PI /4
				// ;//Math.atan((double)(-vz.vy)/(double)vz.vy);
				// alp=alp+Math.PI;
				// alp=alp-Vtask.a;
			...
			stat = false;
			n_Satp = 3;
			if (frustration_cpt < 1)
				ToDetect = TYPE_OBSTACLE + TYPE_ROBOT;
			else
				ToDetect = TYPE_OBSTACLE + TYPE_ROBOT + TYPE_PLAQUE;
		}
			break;
		case 1: { // pousser/tirer
			...
			Vtask.fixN(Ra);
			ToDetect = TYPE_OBSTACLE + TYPE_ROBOT + DONT_CARE_PLAQUE
					+ DONT_CARE_FRIENDS;
			test_agt = 0;
			n_Satp = 1;
		}
		...
		case 2: { // Decouper ...
			...
			if ((AppliquerDecoupe(maPlaque)) == false)
				System.out.println("Moi=" + N_agent
						+ " Oups impuissant sur maPlaque=" + maPlaque);
		...
		case 3: { // Approcher
			...
			if (maPlaque == -1) {
				...
				if (maPlaque != -1) {
					...
					System.out.println("Moi=" + N_agent + " accroche + "
							+ maPlaque + " distance=" + distAccroche);
				}
			}
			if (frustration_cpt < 1)
				ToDetect = TYPE_OBSTACLE + TYPE_ROBOT;
	...
	public String runMe() {
		...
		if ((x != -1) || (y != -1)) {
			...
			Vrad = new Vecteur(radarV(((float) Vtask.a), ouv, (float) Ra,
					(float) Rv, 2, except, (byte) 1, TYPE_OBSTACLE
							+ TYPE_PLAQUE + TYPE_ROBOT));
	...
	public void ExecutionTacheDeplacement() {
			...
			if (Tcurrent == 1) {
				...
				// On a une plaque !!!!!
				{
					...
					if (RobotType == TYPE_POUSSEUR) {
						...
						if ((AppliquerForce(
								Math.round(xPl * PuissanceEnPoussee),
								Math.round(yPl * PuissanceEnPoussee), maPlaque)) == false)
							System.out.println("Moi=" + N_agent
									+ " Oups impuissant sur maPlaque="
									+ maPlaque);
						// System.out.print("ok. ");
					} else if (RobotType == TYPE_DECOUPEUR) {
					...
					System.out.println("Bug RobotBrain.runMe");
				}
			} // if (Tcurrent==1)
			else {
		...
		} // else // if ((xp<Ra) || (yp<Ra) || (xp>(dimx()-Ra)) ||
			// (yp>(dimy()-Ra)) )
		...
		if (dep == false) // == Si dep impossible
		{
			...
			if (Tcurrent == 1) // pousser
				System.out.println("Moi=" + this.N_agent
						+ " pousse mais bloque");
	...
	public int UpdateNumPlaque(int numP) {
		if ((numP != -1) && (!Exist(numP))) {
			System.out.println("Moi=" + N_agent + " Decrochage force'de "
					+ maPlaque);
			maPlaque = -1;
			distAccroche = -1;
			return -1;
		...
		if ((numP != -1) && ((d > distAccroche + varDecroche)) // ||(d<distAccroche-(2*varDecroche))
																// )
		) {
			System.out.println("Moi=" + N_agent + " s'est fait largue' par "
					+ maPlaque + " " + distAccroche + "/" + d);
			DecrocherPlaque();
			maPlaque = -1;
			distAccroche = -1;
			return -1;
		 ...
		 * DecrocherPlaque(); maPlaque=-1; distAccroche=-1; return -1; }
		 */
		return numP;
